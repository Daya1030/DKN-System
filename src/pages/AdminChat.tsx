...
export default function AdminChat() {
  ...
  const canAccess = role === 'Administrator' || role === 'KnowledgeChampion'
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem('dkn:admin-chat')
      if (saved) {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setMessages(updated)
    localStorage.setItem('dkn:admin-chat', JSON.stringify(updated))
    setNewMessage('')
    push('✓ Message sent')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', paddingBottom: '40px' }}>
      {/* Header */}
      <section style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', padding: '40px 24px', marginBottom: '32px', borderRadius: '0' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <p style={{ margin: 0, color: 'rgba(255,255,255,0.9)' }}>
            Direct communication between Knowledge Champions and Administrators
          </p>
        </div>
      </section>
                    ...
                    >
                      <div
                        style={{
                          ...
                          boxShadow: isCurrentUser ? '0 2px 8px rgba(102,126,234,0.2)' : '0 1px 3px rgba(0,0,0,0.05)'
                        }}
                      >
                        <div style={{
                          fontSize: '0.85rem',
                          ...
                          <span style={{
                            ...
                            {msg.senderRole === 'Administrator' ? '⚙️ Admin' : '⭐ KC'}
                          </span>
                        </div>
                        <div style={{
                          fontSize: '0.95rem',
